...
def main(c=1.0, eps=0.01, a1_range=np.linspace(-1.0, 1.0, 200), a2_range=np.linspace(-1.0, 1.0, 200)):
    ...
    with multiprocessing.Pool() as pool:
        ...
        results = list(tqdm(pool.imap(calculate_phase_diagram_point, params_list), total=len(params_list))) # adds progress bar
...
if __name__ == "__main__":
    ...
    ax.set_title('Total synchronization measure')
    ax.set_xlabel('a1')
    ax.set_ylabel('a2')
